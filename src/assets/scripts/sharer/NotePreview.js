...
export default class NotePreview extends Component {
  previewContent() {
    ...
    const handleLink = (handle) => {
      if (handle.charAt(0) === '@') {
        return <a href={`https://twitter.com/${handle.substring(1)}`}>{handle}</a>;
      }
      return handle;
    };
    const viaLink = via.trim().length ? <span> (via {handleLink(via)})</span> : null;
  ...
  renderMeta() {
    const date = new Date();
    const dateString = DateTime.fromJSDate(date).toFormat('dd LLL yyyy - HH:mm');

    return (
      <div className='note__meta'>
        <p className='note__author' hidden>
          <span className='note__author__link'>
            <img className='note__author__photo' src={assetUrls.avatar} />
            <strong className='note__author__name'>Sadanand Singh</strong>
          </span>
        </p>
