...
export default class Webmention extends Component {
  ...
  render({ id, author, published, content }) {
    ...
    const ownDomains = ['https://reckoning.dev', 'https://twitter.com/reckoningdev'];
    ...
    return (
      <div className={classNames.join(' ')} id={`webmention-${id}`}>
        <div className='webmention__meta'>
          {this.renderAuthor()}
          <span className='webmention__meta__divider' aria-hidden='true'>
            &sdot;
          </span>
          <time className='webmention__pubdate dt-published' datetime={published}>
