...
const API_FILE_TARGET =
  'https://api.github.com/repos/sadanand-singh/reckoning.dev/contents/src/notes/';
...
const getFileContent = (data) => {
  ...
  if (via) {
    const vialink =
      via.charAt(0) === '@' ? `[${via}](https://twitter.com/${via.substring(1)})` : via;
    content += ` (via ${vialink})`;
  }
  content += '\n\n' + `[${url}](${url})`;
...
const postFile = async (params) => {
  ...
  const payload = {
    ...
    committer: {
      name: 'Sadanand Singh',
      email: 'sadanand4singh@gmail.com',
    },
  };
